...
public interface UserRepository extends CrudRepository<User, Long> {
    ...
    @Query(value = "SELECT * FROM user_data WHERE chatId = ?1 LIMIT 1", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE user_data SET latitude = ?2, longitude = ?3 WHERE chatId = ?1", nativeQuery = true)
    void updateLocation(long chatId, double latitude, double longitude);

    @Query(value = "SELECT CASE WHEN COUNT(*) > 0 THEN true ELSE false END FROM user_data WHERE chatId = ?1", nativeQuery = true)
    boolean isUserExists(long chatId);

    @Modifying
    @Transactional
    @Query(value = "UPDATE user_data SET time = ?2 WHERE chatId = ?1", nativeQuery = true)
    void updateTime(long chatId, LocalTime time);

    @Query(value = "SELECT latitude FROM user_data WHERE chatId = ?1", nativeQuery = true)
    Double findLatitude(long chatId);

    @Query(value = "SELECT longitude FROM user_data WHERE chatId = ?1", nativeQuery = true)
